...
from qucheck.stats.statistical_analysis_coordinator import StatisticalAnalysisCoordinator, TestExecutionStatistics
...
class TestRunner:
    ...
    def run_tests(self, backend=AerSimulator(), run_optimization=True, family_wise_p_value=0.01, mode="TEST", name_mod=0,
                  algorithm_name=None, clf=False, transpile_gates=None, noise_model=None, quri=False) -> TestExecutionStatistics:
        ...
        stat_analysis_coordinator = StatisticalAnalysisCoordinator(self.num_measurements, family_wise_p_value, mode)
        properties = []
        ...
        if mode == "HYBRID":
            self.test_execution_stats = stat_analysis_coordinator.perform_analysis_hybrid(properties, backend, run_optimization,
                                                                                          name_mod=name_mod, algorithm_name=algorithm_name,
                                                                                          clf=clf, transpile_gates=transpile_gates,
                                                                                          noise_model=noise_model, quri=quri)
        elif mode == "DATA_FARM":
            self.test_execution_stats = stat_analysis_coordinator.perform_analysis_data_farm(properties, backend, run_optimization,
                                                                                             name_mod=name_mod, algorithm_name=algorithm_name,
                                                                                             clf=clf, transpile_gates=transpile_gates,
                                                                                             noise_model=noise_model, quri=quri)
        else:
